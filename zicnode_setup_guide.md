...
*   **Thêm Node vào VPS hiện tại:** `zicnode add-node`
*   **Liệt kê các Node đã cấu hình:** `zicnode nodes`
*   **Mở toàn bộ cổng mạng (Tường lửa):** `zicnode open_ports` (Hữu ích khi bạn bị chặn kết nối từ bên ngoài).

### 3.1. Chạy nhiều Node trên cùng một VPS

ZicNode hỗ trợ nhiều Node trong cùng một tiến trình và cùng một `zicnode.service`. Trên ZicBoard, hãy tạo Node thứ hai với `NodeID` và cổng lắng nghe khác Node đầu tiên, sau đó chạy:
...
Lệnh `add-node` sẽ xác thực Node với panel, sao lưu `/etc/zicnode/config.json`, thêm phần tử mới vào `Nodes[]` và tự phục hồi cấu hình cũ nếu dịch vụ không khởi động lại được. Lệnh `nodes` luôn che `ApiKey` khi hiển thị.

Cấu hình sau khi thêm Node thứ hai có dạng:
...
Hai Node không được dùng cùng cổng lắng nghe trên cùng địa chỉ IP. Nếu hai Node thuộc hai panel khác nhau nhưng trùng `NodeID`, hãy cấu hình đường dẫn certificate riêng để tránh dùng chung tệp `node-<id>.cer` và `node-<id>.key`.
